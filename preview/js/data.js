...
const SubManager = {
    ...
    token: '',
    subsCache: null,
    setAuth: function(apiBase, token) {
    ...
    getSubs: function() {
        ...
        if (changed) {
            try { localStorage.setItem(this.key, JSON.stringify(subs)); } catch (e) {}
        }
        this.subsCache = subs;
    ...
    saveSubs: function(subs) {
        this.subsCache = subs || [];
        try { localStorage.setItem(this.key, JSON.stringify(this.subsCache)); } catch (e) {}
    },
    refreshFromServer: async function() {
        ...
        try {
            ...
            if (resp.ok && json && json.code === 1 && Array.isArray(json.data)) {
                this.saveSubs(json.data);
                if (localSubs.length) {
                    localSubs.forEach(s => {
                        try {
                            this.addSub({
                                keywords: s.keywords || [],
                                scopeType: s.scopeType || 'country',
